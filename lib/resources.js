...
/**
 * Fence permits & HOA approval content cluster.
...
export function resourceJsonLd(resource) {
  const pageUrl = `${SITE.url}${resourcePath(resource.slug)}`;
  const webpage = {
    "@type": "WebPage",
    "@id": `${pageUrl}#webpage`,
    url: pageUrl,
    ...
    about: {
      "@type": "Thing",
      name: "Fence permits and HOA approval in Florida",
    },
    provider: {
      "@type": "LocalBusiness",
      name: SITE.name,
  ...
  const service = {
    "@type": "Service",
    name: "Vinyl and chain-link fence installation with permit and HOA documentation help",
    description: resource.description,
    provider: {
      "@type": "LocalBusiness",
      name: SITE.name,
  ...
  const faqPage =
    resource.faqs && resource.faqs.length
      ? {
          "@type": "FAQPage",
          "@id": `${pageUrl}#faq`,
  ...
  return {
    ...
    "@graph": [webpage, service, ...(faqPage ? [faqPage] : [])],
  };
}

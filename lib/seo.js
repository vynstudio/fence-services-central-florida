 ...
 * Markets: Jacksonville, Orlando, Kissimmee.
 */
...
/**
 * Homepage JSON-LD — full graph per SEO/AEO implementation brief (Jul 25, 2026).
 * FAQ answers must match HOME_FAQS on-page text.
 */
export function getHomeJsonLd() {
  const logoUrl = abs(SITE.logo);
  const imageUrl = abs(SITE.logo);
  ...
  return {
    ...
    "@graph": [localBusiness, website, organization, faqPage],
  };
}
